# -*- coding: utf-8 -*-
"""
Created on Mon Dec 22 21:47:47 2025
...
def draw_scatter(x,y,filename):
    ...
    b = a
    plt.plot(a,b)
    plt.text(8,1,'r2=%.3f'%r2, fontsize=12, color='k')
    plt.text(8,0,'RMSE=%.3f'%rmse, fontsize=12, color='k')
...
def optuna_opt(data):
    X = data[['x6','x7','x8','x9','x10']]
    Y = data[['Mass_activity']]
    x = X.to_numpy()
    y = Y.to_numpy()
    ...
    def func(hidden_layers,hidden_units,dropout_rate,activation,lr,optimizer_name,epoches):
        model = FNNRegressor2(x_train_tensor.shape[1],hidden_layers,hidden_units,dropout_rate,activation)
    ...
    def model_train(para):
        ...
        model = FNNRegressor2(x_train_tensor.shape[1],best_para[0],best_para[1],best_para[2],best_para[3])
        ...
        for i in range(epoches):
            ...
            loss.backward()
            optimizer.step()
        torch.save(model, 'NN_net_2.pkl')
        model.eval()
        y_test_pred = model(x_test_tensor).squeeze().detach().numpy()
        y_train_pred = model(x_train_tensor).squeeze().detach().numpy()
        y_test_real = y_test.squeeze()
        y_train_real = y_train.squeeze()
        draw_scatter(y_train_real,y_train_pred,'Training set of neural network model 2')
        draw_scatter(y_test_real,y_test_pred,'Testing set of neural network model 2')
...
if __name__ == "__main__":
    ...
    para.to_csv('NN_best_para_2.csv', index=False)